          ...
          >
            ...
            <!-- <img src="https://placehold.it/200x200" /> -->
              ...
              <img :src="video.thumbnail" >
                <p>{{ video.subject }}</p>
                <p class="tag"> 조회수: {{video.view_num}}</p>
                <p class="tag"> 좋아요수: {{video.like_num}}</p>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    atEndOfList() {
      return (
        this.currentOffset <=
        this.paginationFactor * -1 * (this.videolist.length - this.windowSize)
      );
    },
    atHeadOfList() {
  ...
  methods: {
    ...
    getVideoList() {
      ...
      axios
        ...
        .catch((exp) => alert("selectAll처리에 실패하였습니다." + exp));
    },
  },
};
</script>
